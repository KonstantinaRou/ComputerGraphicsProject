...
float is_point_in_shadow_pcf_5()
{
	...
	*/
	...
	if (position_lcs.z <= shadow_map_z + constant_depth_bias)
		total_sum++;
	shadow_map_z = texture(uniform_sampler_shadow_map, position_lcs.xy+ vec2(+shadow_map_step,0)).r;
	if (position_lcs.z <= shadow_map_z + constant_depth_bias)
		total_sum++;
	shadow_map_z = texture(uniform_sampler_shadow_map, position_lcs.xy+ vec2(-shadow_map_step,0)).r;
	if (position_lcs.z <= shadow_map_z + constant_depth_bias)
		total_sum++;
		shadow_map_z = texture(uniform_sampler_shadow_map, position_lcs.xy+ vec2(0,+shadow_map_step)).r;
	if (position_lcs.z <= shadow_map_z + constant_depth_bias)
		total_sum++;
			shadow_map_z = texture(uniform_sampler_shadow_map, position_lcs.xy+ vec2(0,-shadow_map_step)).r;
	if (position_lcs.z <= shadow_map_z + constant_depth_bias)
